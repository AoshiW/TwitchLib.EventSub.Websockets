...
namespace TwitchLib.EventSub.Websockets.Handler.Channel.Goals
{
    ...
    public class ChannelGoalProgressHandler : INotificationHandler
    {
        ...
        public void Handle(EventSubWebsocketClient client, string jsonString, WebsocketsEventSubMetadata metadata, JsonSerializerOptions serializerOptions)
        {
            try
            {
                var data = JsonSerializer.Deserialize<EventSubNotificationPayload<ChannelGoalProgress>>(jsonString.AsSpan(), serializerOptions);

                if (data is null)
                    throw new InvalidOperationException("Parsed JSON cannot be null!");

                client.RaiseEvent("ChannelGoalProgress", new ChannelGoalProgressArgs { Payload = data, Metadata = metadata });
            }
            catch (Exception ex)
            {
