...
namespace TwitchLib.EventSub.Websockets.Handler.Channel.SuspiciousUser
{
    ...
    public class ChannelSuspiciousUserUpdateHandler : INotificationHandler
    {
        ...
        public void Handle(EventSubWebsocketClient client, string jsonString, WebsocketsEventSubMetadata metadata, JsonSerializerOptions serializerOptions)
        {
            try
            {
                var data = JsonSerializer.Deserialize<EventSubNotificationPayload<ChannelSuspiciousUserUpdate>>(jsonString.AsSpan(), serializerOptions);

                if (data is null)
                    throw new InvalidOperationException("Parsed JSON cannot be null!");

                client.RaiseEvent("ChannelSuspiciousUserUpdate", new ChannelSuspiciousUserUpdateArgs { Payload = data, Metadata = metadata });
            }
            catch (Exception ex)
            {
