...
namespace TwitchLib.EventSub.Websockets.Handler.Channel.GuestStar
{
    ...
    public class ChannelGuestStarGuestUpdateHandler : INotificationHandler
    {
        ...
        public void Handle(EventSubWebsocketClient client, string jsonString, WebsocketsEventSubMetadata metadata, JsonSerializerOptions serializerOptions)
        {
            try
            {
                var data = JsonSerializer.Deserialize<EventSubNotificationPayload<ChannelGuestStarGuestUpdate>>(jsonString.AsSpan(), serializerOptions);

                if (data is null)
                    throw new InvalidOperationException("Parsed JSON cannot be null!");

                client.RaiseEvent("ChannelGuestStarGuestUpdate", new ChannelGuestStarGuestUpdateArgs { Payload = data, Metadata = metadata });
            }
            catch (Exception ex)
            {
