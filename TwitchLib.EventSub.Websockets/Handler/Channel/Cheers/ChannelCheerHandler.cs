...
namespace TwitchLib.EventSub.Websockets.Handler.Channel.Cheers
{
    ...
    public class ChannelCheerHandler : INotificationHandler
    {
        ...
        public void Handle(EventSubWebsocketClient client, string jsonString, WebsocketsEventSubMetadata metadata, JsonSerializerOptions serializerOptions)
        {
            try
            {
                var data = JsonSerializer.Deserialize<EventSubNotificationPayload<ChannelCheer>>(jsonString.AsSpan(), serializerOptions);

                if (data is null)
                    throw new InvalidOperationException("Parsed JSON cannot be null!");

                client.RaiseEvent("ChannelCheer", new ChannelCheerArgs { Payload = data, Metadata = metadata });
            }
            catch (Exception ex)
            {
