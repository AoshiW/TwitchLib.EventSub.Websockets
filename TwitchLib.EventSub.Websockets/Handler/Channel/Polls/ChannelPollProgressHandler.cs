...
namespace TwitchLib.EventSub.Websockets.Handler.Channel.Polls
{
    ...
    public class ChannelPollProgressHandler : INotificationHandler
    {
        ...
        public void Handle(EventSubWebsocketClient client, string jsonString, WebsocketsEventSubMetadata metadata, JsonSerializerOptions serializerOptions)
        {
            try
            {
                var data = JsonSerializer.Deserialize<EventSubNotificationPayload<ChannelPollProgress>>(jsonString.AsSpan(), serializerOptions);

                if (data is null)
                    throw new InvalidOperationException("Parsed JSON cannot be null!");

                client.RaiseEvent("ChannelPollProgress", new ChannelPollProgressArgs { Payload = data, Metadata = metadata });
            }
            catch (Exception ex)
            {
